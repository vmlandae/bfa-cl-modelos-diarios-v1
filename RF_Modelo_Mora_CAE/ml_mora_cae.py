...
RUTA_INTERFAZ_DE_DATOS = cr.resolver_ruta(config_ext['modelos']['ml_mora_cae']['interfaz_datos_input'])
RUTA_PARAMETOS_MORA_CAE = cr.resolver_ruta(config_ext['modelos']['ml_mora_cae']['excel_parametros_input'])
RUTA_OUTPUT_MODELO = cr.resolver_ruta(config_ext['modelos']['ml_mora_cae']['excel_output'])
...
def lectura_parametros_modelo():
    ...
    Returns:
        tuple: (factores_mora, matriz_mora_cae, factores_globales_mora)
            - factores_mora: DataFrame con factores específicos de mora
...
def lectura_interfaz_de_datos(fecha_t: datetime.datetime)-> pd.DataFrame:
    """
    Lee y procesa los datos de la interfaz para una fecha específica.
    
    Args:
        fecha_t (datetime.datetime): Fecha de proceso para la cual se leerán los datos
    
    Returns:
        pd.DataFrame: DataFrame con los datos filtrados y procesados para CRUGE.
        Incluye las siguientes columnas principales:
        - FECHA_PROCESO
        - SISTEMA
        - CODIGO_SUBPRODUCTO
        - DESTINOCREDITO
...
def procesamiento_y_guardado(fecha_t: datetime.datetime,
    ...
    print("      • Preparando datos CAE..")
    credito_cae = interfaz_de_datos_t
    
    print("      • Cargando parámetros del modelo...")
    factor_mora_credito_cae, matriz_mora_credito_cae, fg_cae = lectura_parametros_modelo()
    print(f"        - Factor Global CAE: {fg_cae:.4f}")

    print("      • Calculando flujos estimados de mora...")
    flujo_estimado_iter = calcular_flujos_estimados_mora(
        data=credito_cae,
        fecha_t=fecha_t,
        matriz_mora=matriz_mora_credito_cae,
        factor_mora=factor_mora_credito_cae,
        factor_global=fg_cae)
    ...
    tabla_desarrollo_tmp = {
        ...
        "CODIGO_SUBPRODUCTO": ["ML_C46_MORA_CREDITO_COMERCIAL"] * registros,
    ...
    flujo_estimado_iter["PRODUCTO"] = "CAE"
    ...
    #                          str(cr.EJECUCIONES_ML_MORA_CAE),
...
def ejecutar_modelo(fecha_proceso: datetime.datetime) -> bool:
    """
    Función principal que ejecuta todo el flujo del modelo de mora CAE.
    ...
    try:
        print("\n" + "="*50)
        print("INICIO DEL PROCESO - MODELO MORA CAE")
...
if __name__ == "__main__":
    ...
    Uso desde línea de comandos:
        python ml_mora_cae.py YYYY-MM-DD
    
    Ejemplo:
        python ml_mora_cae.py 2025-10-18
    
    El script realiza el siguiente flujo:
    1. Validación de la fecha de entrada
    2. Lectura de datos de interfaz
    3. Procesamiento y cálculo de flujos de mora
    4. Generación y guardado de reportes
    """
    
    if len(sys.argv) < 2:
        print("ERROR: No se proporcionó fecha. Uso: python ml_mora_cae.py YYYY-MM-DD")
        sys.exit(1)
