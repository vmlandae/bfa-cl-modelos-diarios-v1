"""
SCRIPT DE DESARROLLO -- NO USADO POR EL ORQUESTADOR.

Esta es la version anterior del modelo Prepago CMR, parametrizada via
`parametros_mr_prepago_cmr.xlsx` y con varias divergencias funcionales
respecto al notebook productivo (manejo de Dia_F 28/29, calendario de
200 meses, exclusion de cuotas MORA vencidas, SMM sin division por 100,
CODIGO_EMPRESA = 1, escenarios desde Excel).

Se conserva como base para retomar las mejoras (parametrizacion,
escenarios desde config, etc.) en una rama de desarrollo, una vez
validadas las divergencias con metodologias.

El modelo activo es `mr_prepago_cmr.py` (replica fiel del notebook).
Ver `docs/feats/cuadre-mr-prepago-cmr/hallazgos.md`.
"""
import pandas as pd
import numpy as np
