...
def procesamiento_y_guardado(
    ...
    for sub_producto in interfaz_hip_t["GLOSA_CODIGO_SUBPRODUCTO"].unique():
        ...
        for esc in escenarios.keys():
            ...
            tabla_desarrollo.to_clipboard(sep=';',decimal=",", index=False)
    ...
    # print("          ✓ Copia de respaldo creada")
             ...
             # info_escenario = resultados['INFO_ESCENARIO']
            # nombre_archivo = f"Resultados_{sub_producto}_{info_escenario['DESCRIPCION']}.xlsx"
            # ruta = os.path.join("Z:\RF_PROYECTOS\METODOLOGIAS\PROCESOS_DIARIOS_MODELOS\MANUALES\MODELOS\MR_PREPAGO_HIPOTECARIO", nombre_archivo)
            # with pd.ExcelWriter(ruta) as writer:
            #     resultados['MATRIZ_F'].to_excel(writer, sheet_name='Matriz_F')
            #     resultados['FLUJOS'].to_excel(writer, sheet_name='Flujos_de_Caja')
